...
impl Game {
    ...
    pub fn update(&mut self, is_swing: &Mutex<bool>) -> bool {
        ...
        *is_swing = false;
        ...
        let out_right = self.ball > 1.0 + HALF_PADDLE_SIZE;
        !out_left && !out_right
    }

    fn strike_back(&mut self) {
        let is_left = self.ball < 0.5;
        let judgement: f64 = if is_left {
            self.ball.abs()
        } else {
            (self.ball.abs() - 1.0).abs()
        };
        let is_perfect_timing: bool = judgement < HALF_PADDLE_SIZE / 2.0;
